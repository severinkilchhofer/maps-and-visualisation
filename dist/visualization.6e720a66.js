...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var svg = container.append("svg").attr("width", width).attr("height", height).style("border", "1px dashed #000");
  d3.json("/countries.json").then(function (countries) {
    // Different Projections:
    // https://github.com/d3/d3-geo#azimuthal-projections
    var projection = d3.geoBonne().rotate([-20.0, 0.0]).center([0.0, 52.0]).parallel(52).translate([width / 2, height / 2]).scale(580).precision(.1);
    var path = d3.geoPath().projection(projection);
    var europe = svg.selectAll("path").data(countries.features).enter().append("path").attr("d", function (d) {
      return path(d);
